...
public class putRequest extends config {
	...
	public void putReq(String id, String onln, String progDesc, String progName ) throws Exception
	{
	...
	void putData(String progId) throws IOException
	{
		...
		//int cellNum = excelUtil.getCellCount(path, "Delete",rowNum);
		excelUtil.SetCellData(path, "Delete", rowNum, 0, progId);
	}
	...
	Object[][] getData() throws IOException
	{
		...
		int cellNum = excelUtil.getCellCount(path, "Put", rowNum);
		System.out.println(rowNum + "====" + cellNum);
		String [][] data = new String [rowNum][cellNum];
		for(int i=1;i<=rowNum;i++)
			for(int j=0;j<cellNum;j++)
			{
				data[i-1][j] = excelUtil.GetCellData(path, "Put", i, j);
